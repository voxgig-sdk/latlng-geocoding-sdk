...
// LatlngGeocoding SDK retry feature
...
class LatlngGeocodingRetryFeature extends LatlngGeocodingBaseFeature
{
    ...
    public function init(LatlngGeocodingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LatlngGeocodingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LatlngGeocodingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LatlngGeocodingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
