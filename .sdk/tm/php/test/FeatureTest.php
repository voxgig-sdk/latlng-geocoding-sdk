...
// LatlngGeocoding SDK feature test
//
...
class FtCtrl extends LatlngGeocodingControl
{
...
class FtClient
{
    ...
    private LatlngGeocodingUtility $utility;

    public function __construct(LatlngGeocodingUtility $utility, array $options)
    {
    ...
    public function get_utility(): LatlngGeocodingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LatlngGeocodingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LatlngGeocodingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LatlngGeocodingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LatlngGeocodingConfig::make_feature($name);
    ...
    private static function buildUrl(LatlngGeocodingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LatlngGeocodingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LatlngGeocodingResponse($fetched);
        }
    ...
    private function fail(LatlngGeocodingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LatlngGeocodingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
