...
// LatlngGeocoding SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class LatlngGeocodingMakeError
{
    public static function call(?LatlngGeocodingContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new LatlngGeocodingContext([], null);
        }
        $op = $ctx->op ?? new LatlngGeocodingOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new LatlngGeocodingResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof LatlngGeocodingError) ? $err->msg : (string)$err;
        $msg = "LatlngGeocodingSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new LatlngGeocodingError('', $msg, $ctx);
        ...
        if ($err instanceof LatlngGeocodingError) {
