...
module LatlngGeocodingUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = LatlngGeocodingContext.new({}, nil)
    end
    op = ctx.op || LatlngGeocodingOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || LatlngGeocodingResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(LatlngGeocodingError) ? err.msg : err.to_s
    ...
    sdk_err = LatlngGeocodingError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(LatlngGeocodingError)
