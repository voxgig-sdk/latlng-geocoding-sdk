...
module LatlngGeocodingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LatlngGeocodingResponse.new({})
    ...
    if fetch_err
      ...
      response = LatlngGeocodingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LatlngGeocodingResponse.new(fetched)
    else
