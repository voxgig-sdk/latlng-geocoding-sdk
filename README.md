...
LatLng API client, generated from the OpenAPI spec.

LatLng provides affordable maps and location APIs for developers building location-aware apps.

Learn more about LatLng API at [www.latlng.work/docs](https://www.latlng.work/docs).

This is an unofficial SDK for the LatLng public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Api, Dataset, Map, Place, Reverse and Utility — that you
...
const items = await client.Api().list({ q: "example" })
```
...
```bash
cd go-mcp && go build -o latlng-geocoding-mcp .
```
...
```json
{
  "mcpServers": {
    "latlng-geocoding": {
      "command": "/abs/path/to/latlng-geocoding-mcp"
    }
  }
}
```
...
| **Utility** | The Utility entity (load). | `/health` |
...
```go
import sdk "github.com/voxgig-sdk/latlng-geocoding-sdk/go"
...
Generated from the LatLng API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
