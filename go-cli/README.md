# latlng-geocoding-cli

boru-driven command-line client **and** interactive REPL for the LatlngGeocoding
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/latlng-geocoding-cli)
make build

# 2. See usage (words, entities, env vars)
./latlng-geocoding-cli --help

# 3. Provide credentials once, via the environment
export LATLNG_GEOCODING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./latlng-geocoding-cli list api
./latlng-geocoding-cli list dataset

# 5. Override the API base URL for a single call
LATLNG_GEOCODING_BASE=https://api.example.com ./latlng-geocoding-cli list api

# 6. No arguments -> interactive REPL
./latlng-geocoding-cli
latlng-geocoding> list api
latlng-geocoding> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/latlng-geocoding-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/latlng-geocoding-cli list api
   ```
...
Run with no arguments to open a REPL (prompt `latlng-geocoding>`). Each line is
evaluated as its own boru expression:

```text
$ ./latlng-geocoding-cli
latlng-geocoding> list api
latlng-geocoding> /help
latlng-geocoding> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/latlng-geocoding-cli`. |
...
(`./latlng-geocoding-cli <expr>`) and interactively (the REPL), and expressions compose the
