// Command LatlngGeocoding-cli is a CLI + REPL for the LatlngGeocoding SDK,
...
const prompt = "latlng-geocoding"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "api dataset map place reverse utility"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LATLNG_GEOCODING_APIKEY (api key), LATLNG_GEOCODING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LATLNG_GEOCODING_APIKEY carries the API key and
	// LATLNG_GEOCODING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LATLNG_GEOCODING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LATLNG_GEOCODING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLatlngGeocodingSDK(opts)
