# latlng-geocoding-mcp

[MCP](https://modelcontextprotocol.io) server exposing the LatlngGeocoding SDK as
two agent tools — `latlng-geocoding_list` and `latlng-geocoding_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/latlng-geocoding-mcp)
make build

# 2. Provide credentials via the environment
export LATLNG_GEOCODING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user latlng-geocoding \
  -- /absolute/path/to/latlng-geocoding-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./latlng-geocoding-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// latlng-geocoding_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// latlng-geocoding_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/latlng-geocoding-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user latlng-geocoding \
     -- "$PWD"/dist/*/latlng-geocoding-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `latlng-geocoding_list` and `latlng-geocoding_load` tools now appear
   in new sessions. Ask the agent to *"list api using latlng-geocoding"*
   and it calls `latlng-geocoding_list` with `{"entity":"api"}`.
...
```sh
export LATLNG_GEOCODING_APIKEY=sk_live_xxx            # API key
export LATLNG_GEOCODING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `latlng-geocoding_list` tool
...
### Call the `latlng-geocoding_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `latlng-geocoding_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `latlng-geocoding_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LATLNG_GEOCODING_APIKEY` | API key sent with every request. |
| `LATLNG_GEOCODING_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

api | dataset | map | place | reverse | utility

### Smoke test via HTTP (raw JSON-RPC)

```sh
./latlng-geocoding-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"latlng-geocoding_load","arguments":{"entity":"dataset","query":{"id":1}}}}'
```
