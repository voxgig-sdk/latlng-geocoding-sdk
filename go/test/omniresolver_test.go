...
import (
	...
	omni "github.com/voxgig-sdk/latlng-geocoding-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LatlngGeocodingError); is {
				out := map[string]any{
					"name":    "LatlngGeocodingError",
...
func omniCtx(arg any, client *sdk.LatlngGeocodingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LatlngGeocodingSDK); is {
			client = live
