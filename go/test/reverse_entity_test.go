...
func TestReverseEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Reverse(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Reverse(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LATLNGGEOCODING_TEST_REVERSE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func reverseBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LATLNGGEOCODING_TEST_REVERSE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LATLNGGEOCODING_TEST_REVERSE_ENTID"])
