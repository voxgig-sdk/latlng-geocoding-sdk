# LatlngGeocoding Lua SDK



The Lua SDK for the LatlngGeocoding API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Api()` — each with the same small set of operations (`list`, `load`, `create`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/latlng-geocoding-sdk/releases)),
...
The main module (`latlng-geocoding_sdk`) exports the SDK constructor
