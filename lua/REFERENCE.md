# LatlngGeocoding Lua SDK Reference

Complete API reference for the LatlngGeocoding Lua SDK.
...
```lua
local result, err = client:Dataset():remove({ id = "dataset_id" })
```
...
```lua
local result, err = client:Map():load()
```
