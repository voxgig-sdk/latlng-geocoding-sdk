...
require("latlng-geocoding_types")
...
local LatlngGeocodingSDK = {}
LatlngGeocodingSDK.__index = LatlngGeocodingSDK
...
LatlngGeocodingSDK._make_feature = _make_feature


function LatlngGeocodingSDK.new(options)
  local self = setmetatable({}, LatlngGeocodingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function LatlngGeocodingSDK:options_map()
...
function LatlngGeocodingSDK:get_utility()
  return Utility.copy(self._utility)
end


function LatlngGeocodingSDK:get_root_ctx()
  return self._rootctx
end


function LatlngGeocodingSDK:prepare(fetchargs)
...
function LatlngGeocodingSDK:direct(fetchargs)
...
function LatlngGeocodingSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function LatlngGeocodingSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "LatlngGeocodingSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function LatlngGeocodingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function LatlngGeocodingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "LatlngGeocodingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function LatlngGeocodingSDK:Api(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LatlngGeocodingSDK:Dataset(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LatlngGeocodingSDK:Map(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LatlngGeocodingSDK:Place(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LatlngGeocodingSDK:Reverse(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function LatlngGeocodingSDK:Utility(data)
...
function LatlngGeocodingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = LatlngGeocodingSDK.new(sdkopts)
  sdk.mode = "test"
