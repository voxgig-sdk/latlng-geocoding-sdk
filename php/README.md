# LatlngGeocoding PHP SDK



The PHP SDK for the LatlngGeocoding API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Api()` — with named operations (`list`/`load`/`create`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/latlng-geocoding-sdk/releases](https://github.com/voxgig-sdk/latlng-geocoding-sdk/releases)
...
```php
<?php
require_once 'latlnggeocoding_sdk.php';

$client = new LatlngGeocodingSDK([
    "apikey" => getenv("LATLNG_GEOCODING_APIKEY"),
]);
```
...
```php
$client = LatlngGeocodingSDK::test($testopts, $sdkopts);
```
...
The main class (`latlnggeocoding_sdk.php`) exports the SDK class
