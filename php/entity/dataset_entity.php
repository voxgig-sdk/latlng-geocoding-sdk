...
class DatasetEntity
{
    ...
    {
        if ($args) {
            $this->_data = LatlngGeocodingHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = LatlngGeocodingHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws LatlngGeocodingError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = LatlngGeocodingHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param DatasetRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; DatasetRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Dataset|array The removed Dataset as an assoc-array at the
     *   SDK boundary; throws LatlngGeocodingError on failure (item-5 convention).
     */
