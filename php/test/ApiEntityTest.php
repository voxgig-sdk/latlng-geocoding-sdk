...
class ApiEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LATLNGGEOCODING_TEST_API_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function api_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LATLNGGEOCODING_TEST_API_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "LATLNGGEOCODING_TEST_API_ENTID" => $idmap,
    ...
    $idmap_resolved = Helpers::to_map(
        $env["LATLNGGEOCODING_TEST_API_ENTID"]);
