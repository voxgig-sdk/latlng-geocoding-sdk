...
//    only - a typed LatlngGeocodingContext there would make every ctx
...
final class LatlngGeocodingOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new LatlngGeocodingContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new LatlngGeocodingSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new LatlngGeocodingResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new LatlngGeocodingError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new LatlngGeocodingResponse($clean['response']);
    ...
    public static function err_from_map($m): ?LatlngGeocodingError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new LatlngGeocodingError(is_string($code) ? $code : '', $msg);
    }
