...
// LatlngGeocoding SDK pipeline test
//
...
{
    private static function utility(): LatlngGeocodingUtility
    {
        return new LatlngGeocodingUtility();
    }
    ...
    private static function ctx(array $over = []): LatlngGeocodingContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new LatlngGeocodingContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new LatlngGeocodingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof LatlngGeocodingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new LatlngGeocodingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = LatlngGeocodingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new LatlngGeocodingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = LatlngGeocodingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new LatlngGeocodingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = LatlngGeocodingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LatlngGeocodingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = LatlngGeocodingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new LatlngGeocodingResponse([]),
            'result' => new LatlngGeocodingResult([]),
        ]);
        [, $err] = LatlngGeocodingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new LatlngGeocodingSpec([]),
            'response' => null,
            'result' => new LatlngGeocodingResult([]),
        ]);
        [, $err] = LatlngGeocodingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new LatlngGeocodingSpec(['step' => 's']),
            'response' => new LatlngGeocodingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new LatlngGeocodingResult([]),
        ]);
        [, $err] = LatlngGeocodingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new LatlngGeocodingSpec(['step' => 's']),
            'response' => new LatlngGeocodingResponse(self::resp(200, ['v' => 1])),
            'result' => new LatlngGeocodingResult([]),
        ]);
        [, $err] = LatlngGeocodingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new LatlngGeocodingSpec(['step' => 's']),
            'response' => new LatlngGeocodingResponse(self::resp(200, ['v' => 2])),
            'result' => new LatlngGeocodingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        LatlngGeocodingMakeResponse::call($ctx);
    ...
    {
        $preset = new LatlngGeocodingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new LatlngGeocodingSpec([]),
            'response' => new LatlngGeocodingResponse([]),
            'result' => new LatlngGeocodingResult([]),
        ...
        [$got, $err] = LatlngGeocodingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new LatlngGeocodingResult([])]);
        [, $err] = LatlngGeocodingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new LatlngGeocodingSpec([]), 'result' => null]);
        [, $err] = LatlngGeocodingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LatlngGeocodingSpec(['step' => 's']),
            'result' => new LatlngGeocodingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new LatlngGeocodingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LatlngGeocodingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LatlngGeocodingSpec(['step' => 's']),
            'result' => new LatlngGeocodingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new LatlngGeocodingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LatlngGeocodingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LatlngGeocodingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new LatlngGeocodingSpec([]),
            'result' => new LatlngGeocodingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = LatlngGeocodingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LatlngGeocodingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new LatlngGeocodingError('boom', 'boom');
        $utility->fetcher = function (LatlngGeocodingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LatlngGeocodingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = LatlngGeocodingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (LatlngGeocodingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = LatlngGeocodingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(LatlngGeocodingResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        LatlngGeocodingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (LatlngGeocodingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LatlngGeocodingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = LatlngGeocodingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new LatlngGeocodingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new LatlngGeocodingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = LatlngGeocodingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = LatlngGeocodingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = LatlngGeocodingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new LatlngGeocodingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, LatlngGeocodingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new LatlngGeocodingResult(['ok' => false])]);
        $this->expectException(LatlngGeocodingError::class);
        LatlngGeocodingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new LatlngGeocodingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', LatlngGeocodingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        LatlngGeocodingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new LatlngGeocodingBaseFeature();
        $b = new LatlngGeocodingBaseFeature();
        LatlngGeocodingFeatureAdd::call($ctx, $a);
        LatlngGeocodingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): LatlngGeocodingBaseFeature
    {
        $f = new LatlngGeocodingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        LatlngGeocodingFeatureAdd::call($ctx, self::named_feature('a'));
        LatlngGeocodingFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        LatlngGeocodingFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        LatlngGeocodingFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        LatlngGeocodingFeatureAdd::call($ctx, $replace);
        ...
        LatlngGeocodingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): LatlngGeocodingContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new LatlngGeocodingSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LatlngGeocodingPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        LatlngGeocodingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new LatlngGeocodingResponse(['headers' => null]),
            'result' => new LatlngGeocodingResult([]),
        ]);
        LatlngGeocodingResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        LatlngGeocodingResultBody::call($ctx);
