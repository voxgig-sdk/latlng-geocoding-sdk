# LatlngGeocoding SDK test runner
...
class LatlngGeocodingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LatlngGeocodingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LatlngGeocodingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LatlngGeocodingTestRunner.getenv("LATLNG_GEOCODING_TEST_LIVE")
        override = LatlngGeocodingTestRunner.getenv("LATLNG_GEOCODING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LatlngGeocodingTestRunner.getenv(key)
        ...
        explain = LatlngGeocodingTestRunner.getenv("LATLNG_GEOCODING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LatlngGeocodingTestRunner._test_control is not None:
            return LatlngGeocodingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LatlngGeocodingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LatlngGeocodingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LatlngGeocodingTestRunner.load_test_control()
...
def load_env_local():
    LatlngGeocodingTestRunner.load_env_local()


def env_override(m):
    return LatlngGeocodingTestRunner.env_override(m)


def entity_data(v):
    return LatlngGeocodingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LatlngGeocodingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LatlngGeocodingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LatlngGeocodingTestRunner.load_test_control()


def live_client_options():
    return LatlngGeocodingTestRunner.live_client_options()


def live_delay_ms():
    return LatlngGeocodingTestRunner.live_delay_ms()
