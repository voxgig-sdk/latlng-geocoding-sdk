    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LATLNG_GEOCODING_TEST_MAP_ENTID JSON to run live")
        client = setup["client"]
        ...
        map_ref01_data = helpers.to_map(runner.entity_data(map_ref01_ent.create(map_ref01_data, None)))
        assert map_ref01_data is not None
...
def _map_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LATLNG_GEOCODING_TEST_MAP_ENTID"))
