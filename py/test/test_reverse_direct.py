...
class TestReverseDirect:
    ...
    def test_should_direct_list_reverse(self):
        setup = _reverse_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-reverse", "live" if setup["live"] else "unit")
        if _skip:
