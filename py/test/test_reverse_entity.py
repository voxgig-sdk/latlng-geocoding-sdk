...
class TestReverseEntity:
    ...
    def test_should_stream(self):
        ...
        base = LatlngGeocodingSDK.test(seed, None)
        seen = list(base.Reverse(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = LatlngGeocodingSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Reverse(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LATLNGGEOCODING_TEST_REVERSE_ENTID JSON to run live")
        client = setup["client"]
...
def _reverse_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LATLNGGEOCODING_TEST_REVERSE_ENTID"))
