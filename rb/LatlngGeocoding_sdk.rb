...
require_relative 'LatlngGeocoding_types'


class LatlngGeocodingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LatlngGeocodingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LatlngGeocodingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LatlngGeocodingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    LatlngGeocodingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = LatlngGeocodingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = LatlngGeocodingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LatlngGeocodingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => LatlngGeocodingError.new(
        "#{op}_allow",
        "LatlngGeocodingSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue LatlngGeocodingError => err
    ...
    if fetched.is_a?(Hash)
      status = LatlngGeocodingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LatlngGeocodingError.new(
        "graphql_error", "LatlngGeocodingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = LatlngGeocodingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
