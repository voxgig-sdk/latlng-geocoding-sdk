# LatlngGeocoding SDK Dataset entity
...
class DatasetEntity
  ...
  def data_set(args)
    if args
      @_data = LatlngGeocodingHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = LatlngGeocodingHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Dataset, Hash] the loaded Dataset; raises LatlngGeocodingError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LatlngGeocodingHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Dataset, Hash] the created Dataset; raises LatlngGeocodingError on failure
  ...
  # Remove an Dataset matching the given criteria.
  #
  # @param reqmatch [DatasetRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [Dataset, Hash] the removed Dataset; raises LatlngGeocodingError on failure
