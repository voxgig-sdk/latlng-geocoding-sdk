# LatlngGeocoding TypeScript SDK



The TypeScript SDK for the LatlngGeocoding API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Api()` — each with a small set of operations (`list`, `load`, `create`, `remove`)
...
```ts
const client = new LatlngGeocodingSDK({ apikey: '...' })
...
```ts
new LatlngGeocodingSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `LatlngGeocodingSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `LatlngGeocodingSDK.test(testopts?, sdkopts?)` | `LatlngGeocodingSDK` | Create a test-mode client. |
...
| `client` | `client(): LatlngGeocodingSDK` | Return the parent SDK client. |
...
```ts
const map = await client.Map().load()
```
...
latlng-geocoding/
├── src/
│   ├── LatlngGeocodingSDK.ts        # Main SDK class
