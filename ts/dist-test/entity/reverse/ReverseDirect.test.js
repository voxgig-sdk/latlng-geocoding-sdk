...
(0, node_test_1.describe)('ReverseDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.LatlngGeocodingSDK({
    ...
    (0, node_test_1.test)('direct-list-reverse', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-reverse', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.LATLNG_GEOCODING_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.LatlngGeocodingSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.LATLNG_GEOCODING_APIKEY,
        }));
        let idmap = env['LATLNG_GEOCODING_TEST_REVERSE_ENTID'];
    ...
    const client = new __1.LatlngGeocodingSDK({
        base: 'http://localhost:8080',
