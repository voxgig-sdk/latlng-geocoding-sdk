...
const __1 = require("..");
const SDK_NAME = 'LatlngGeocodingSDK';
...
const TEST_SEED = { "entity": { "api": { "test01": { "id": "test01" } }, "dataset": { "test01": { "id": "test01" } }, "map": { "test01": { "id": "test01" } }, "place": { "test01": { "id": "test01" } }, "reverse": { "test01": { "id": "test01" } }, "utility": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+LatlngGeocodingSDK\b/.test(code) ||
        /\bLatlngGeocodingSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LatlngGeocodingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LatlngGeocodingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
